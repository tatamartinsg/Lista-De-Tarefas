...
function inputTask()
    {
        ...
        if (tarefas == '')
        {
            ...
            var new_concluir = document.createTextNode($botao_concluir)
            ...
            new_li.appendChild($botao_concluir)
            $botao_concluir.appendChild(new_concluir)
            $botao_concluir.innerHTML = 'Concluir Tarefa'
            $botao_concluir.classList.add('botao_concluir')
            ...
            $botao_delete.addEventListener('click', () => {removerTask(a)})
            $botao_edite.addEventListener('click', () => {editTask(a)})
            $botao_concluir.addEventListener('click', () => {concluirTask(a)})
...
function editTask(a)
{
    ...
    $button_class2.addEventListener('click',() => {editCreate(a,$li)})

    
}   
function editCreate(a,$li){
    ...
    <span class="glyphicon glyphicon-remove-circle">
        </span> Excluir</button><button  class="botao_editar editar${a}"> 
                <span class="glyphicon glyphicon-pencil">
                </span> Editar
        </button><button class="botao_concluir concluir${a}"> Concluir Tarefa </button>`

    var botao_editar = document.querySelector(`.editar${a}`)
    botao_editar.addEventListener('click',() => {editTask(a)})

    
    var botao_deletar = document.querySelector(`.deletar${a}`)
    botao_deletar.addEventListener('click',() => {removerTask(a)})

    var botao_concluir = document.querySelector(`.concluir${a}`)
    botao_concluir.addEventListener('click', () => {concluirTask(a)})


}
function removerTask(a)
{
